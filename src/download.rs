    ...
    async fn downloads_file_from_single_peer() {
        ...
        let mock_peer_socket_handle = std::thread::spawn(move || {
            ...
            socket
                ...
                .unwrap();
            println!("PEER: After writing piece 0 block 0 response");
            println!("PEER: Before writing piece 0 block 1 response");
            socket
                .write_all(&piece_zero_responses[1].clone().serialise())
                .unwrap();
            println!("PEER: After writing piece 0 block 1 response");
            let _ = socket.read(&mut throwaway_buf).unwrap();
            println!("PEER: After reading piece 1 block 0 request");
            let _ = socket.read(&mut throwaway_buf).unwrap();
            println!("PEER: After reading piece 1 block 1 request");
            socket
                .write_all(&piece_one_responses[0].clone().serialise())
                .unwrap();
            println!("PEER: After writing piece 1 block 0 response");
            socket
                .write_all(&piece_one_responses[1].clone().serialise())
                .unwrap();
            println!("PEER: After writing piece 1 block 1 response");
        });
